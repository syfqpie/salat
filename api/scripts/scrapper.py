...
class PrayerTimeScrapper:
    ...
    def get_prayer_time(self, period=Period.MONTH.value):
        ...
        for by_zone in self.zones:
            ...
            if cur_res.status_code == requests.codes.ok and cur_res_json["status"] == "OK!":
                code_key = cur_res_json["zone"]
                self.scrapped = self.scrapped + [{ f"{code_key}": cur_res_json["prayerTime"] }]
    ...
    def sync_zones(self):
        ...
        def add_new_entry(new_code, new_text, new_state):
        ...
        def patch_entry(new_code, new_text, new_state, id):
        ...
        if self.zones:
            ...
            for zo in self.zones:
                from_db = [db_zone for db_zone in zones_in_db if db_zone[1] == zo["code"]]
                is_no_data = not from_db
                is_need_update = is_no_data or (zo["code"] != from_db[0][1] or
                                                zo["text"] != from_db[0][2] or
                                                zo["state"] != from_db[0][3])
                if not from_db:
                    add_new_entry(zo["code"], zo["text"], zo["state"])
                elif from_db and is_need_update:
                    patch_entry(zo["code"], zo["text"], zo["state"], from_db[0][0])
    ...
    def sync_prayer_times(self):
        ...
        if not current_month_data_db:
            for by_zone_code in self.scrapped:
                ...
                zone_id = [zdb[0] for zdb in self.zones_in_db if zdb[1] == current_zone][0]

                if not zone_id:
                    continue
                
                for daily in by_zone_code[current_zone]:
                    converted_date = datetime.datetime.strptime(daily['date'], '%d-%b-%Y').strftime("%Y-%m-%d")
                    add_new_prayer_time(zone=current_zone,
                                        zone_id=zone_id,
                                        hijri=daily["hijri"],
                                        date=converted_date,
                                        imsak=daily["imsak"],
            ...
            print('\033[93mERROR\033[0m!! Data not synced, current month data is already in database')
